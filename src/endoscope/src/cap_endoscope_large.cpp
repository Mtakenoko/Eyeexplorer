...
cv::Rect set_ROI(const cv::Mat src, const size_t roi_width, const size_t roi_height){    //ROIを設定する
  cv::Rect ROI;
  cv::Mat gray, bin;
...
int main(int argc, char * argv[])
{
  ...
  if (!parse_command_options(
      argc, argv, &depth, &reliability_policy, &history_policy, &show_camera, &freq,
      &width, &height, &roi_width, &roi_height, &device, &movie_mode, &topic))
  {
    return 0;
  }
