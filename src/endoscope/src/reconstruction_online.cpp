...
void callback(const std::shared_ptr<const sensor_msgs::msg::Image> &msg_image, const std::shared_ptr<const geometry_msgs::msg::Transform> &msg_arm, bool show_camera, size_t feature, size_t match, size_t prjMat, rclcpp::Logger logger, std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::Image>> pub, std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::PointCloud2>> pub_pointcloud)
{
  ...
  //アーム姿勢をcv::Mat形式に変換
  static cv::Mat R_keyframe1(3, 3, CV_32FC1), R_keyframe2(3, 3, CV_32FC1), R_frame(3, 3, CV_32FC1);
  static cv::Mat t_keyframe1(3, 1, CV_32FC1), t_keyframe2(3, 1, CV_32FC1), t_frame(3, 1, CV_32FC1);
  ...
  ////  detection 開始  ////
  static cv::Mat dst_keyframe1, dst_keyframe2, descriptor_keyframe1, descriptor_keyframe2;
  static std::vector<cv::KeyPoint> keypoints_keyframe1, keypoints_keyframe2;
  ...
  for (size_t i = 0; i < dmatch12.size(); ++i)
  {
    ...
    { // 一致しているものだけを抜粋
      ...
      match_point1.push_back(keypoints_keyframe1[dmatch12[i].queryIdx].pt);
  ...
  //画像間でのカメラの移動量（運動学より）
  cv::Mat R_arm(3, 3, CV_32FC1), R_arm_change(3, 3, CV_32FC1), t_arm(3, 1, CV_32FC1),  t_arm_change(3, 1, CV_32FC1), t_endo(3, 1, CV_32FC1), R_endo(3, 3, CV_32FC1);
  R_arm = R_frame * R_keyframe1.t(); //カメラの回転変化
  R_arm_change = R_frame * R_keyframe2.t(); //カメラの回転変化
  t_arm = t_frame - t_keyframe1;     //ワールド座標系でのカメラ移動量
  t_arm_change = t_frame - t_keyframe2;     //ワールド座標系でのカメラ移動量
  R_endo = R_arm.t();
  t_endo = R_keyframe1 * t_arm; //(１つ目の画像撮影時の)カメラ座標系に合わせたカメラ移動量
  ...
  if (match_num > 5) //５点アルゴリズムが行えるのに十分対応点があれば
  {
    ...
    { //特徴点の数だけ処理
      cv::Point2f p_1, p_2;
      p_1.x = keypoints_keyframe1[good_dmatch[i].queryIdx].pt.x;
      p_1.y = keypoints_keyframe1[good_dmatch[i].queryIdx].pt.y;
    ...
    for (size_t i = 0; i < match_num; ++i)
    {
      ...
      point3D_result_arm = (R_keyframe1 * point3D_result.reshape(1, 3) + t_keyframe1) / 1000.;
    ...
    cv::drawMatches(dst_keyframe1, keypoints_keyframe1, dst_frame, keypoints_frame, dmatch, cvframe);
    cv::drawMatches(dst_keyframe1, keypoints_keyframe1, dst_frame, keypoints_frame, good_dmatch, cvframe_);
    if (show_camera)
    {
  ...
  transformRotMatToQuaternion(qx, qy, qz, qw,
                              R_arm_change.at<float>(0, 0), R_arm_change.at<float>(0, 1), R_arm_change.at<float>(0, 2),
                              R_arm_change.at<float>(1, 0), R_arm_change.at<float>(1, 1), R_arm_change.at<float>(1, 2),
                              R_arm_change.at<float>(2, 0), R_arm_change.at<float>(2, 1), R_arm_change.at<float>(2, 2));
  //クォータニオンの4つめの要素から回転角を取り出す
  float phi = 2 * std::acos(qw);
  int frame_span = frame_num - frame_key2;
  if (frame_span > 10 && (cv::norm(t_arm_change) > 6. || phi > PI / 360.))
  {
    dst_keyframe1 = dst_keyframe2.clone();
    keypoints_keyframe1 = keypoints_keyframe2;
    descriptor_keyframe1 = descriptor_keyframe2.clone();
