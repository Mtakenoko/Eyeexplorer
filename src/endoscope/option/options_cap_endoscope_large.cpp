...
bool parse_command_options(
  ...
  double * freq, size_t * width, size_t * height, size_t * roi_width, size_t * roi_height, size_t * device, bool * movie_mode,
  std::string * topic)
{
  ...
  if (find_command_option(args, "-h")) {
    ...
    if (roi_width != nullptr && roi_height != nullptr) {
      ss << " -rx ROI_WIDTH and -ry ROI_HEIGHT. Resolution. " << std::endl;
      ss << "    Please type v4l2-ctl --list-formats-ext " << std::endl;
      ss << "    to obtain a list of valid values." << std::endl;
    }
    if (topic != nullptr) {
  ...
  if (roi_width != nullptr && roi_height != nullptr) {
    auto roi_width_str = get_command_option(args, "-rx");
    auto roi_height_str = get_command_option(args, "-ry");
    if (!roi_width_str.empty() && !roi_height_str.empty()) {
      *width = std::stoul(roi_width_str.c_str());
      *height = std::stoul(roi_height_str.c_str());
    }
  }
