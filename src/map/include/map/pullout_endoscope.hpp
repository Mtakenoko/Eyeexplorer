...
private:
    ...
    void estimate_plane(const pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud);
    void estimate_sphere(const pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud);
    float calc_distance(const EndoscopePose endoscopePose); // 平面のパラメータと内視鏡の位置から、内視鏡と平面の距離を測定する
