...
private:
    void topic_callback_(const std::shared_ptr<const geometry_msgs::msg::TransformStamped> &msg_tip,
                         const std::shared_ptr<const sensor_msgs::msg::Image> &msg_colorimage,
                         const std::shared_ptr<const sensor_msgs::msg::Image> &msg_depthimage);
    void input_tip_data(const std::shared_ptr<const geometry_msgs::msg::TransformStamped> &msg_tip);
    void input_colorimage_data(const std::shared_ptr<const sensor_msgs::msg::Image> &msg_colorimage);
    void input_depthimage_data(const std::shared_ptr<const sensor_msgs::msg::Image> &msg_depthimage);
    void calc();
    ...
    rclcpp::Subscription<geometry_msgs::msg::TransformStamped>::SharedPtr subscription_tip_;
                     ...
                     flag_set_tip(false), flag_set_colorimage(false), flag_set_depthimage(false)
{
    ...
    message_filters::Subscriber<geometry_msgs::msg::TransformStamped> sub_arm_(this, "/endoscope_transformstamped");
    message_filters::Subscriber<sensor_msgs::msg::Image> sub_color_image_(this, "/endoscope_image");
    message_filters::Subscriber<sensor_msgs::msg::Image> sub_depth_image_(this, "/endoscope/image/depth");
    message_filters::TimeSynchronizer<geometry_msgs::msg::TransformStamped, sensor_msgs::msg::Image, sensor_msgs::msg::Image> sync_(sub_arm_, sub_color_image_, sub_depth_image_, 5000);
    sync_.registerCallback(std::bind(&Gridmap::topic_callback_, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void Gridmap::topic_callback_(const std::shared_ptr<const geometry_msgs::msg::TransformStamped> &msg_tip,
                              const std::shared_ptr<const sensor_msgs::msg::Image> &msg_colorimage,
                              const std::shared_ptr<const sensor_msgs::msg::Image> &msg_depthimage)
{
    std::cout << "hello!!!!" << std::endl;
    this->input_tip_data(msg_tip);
    this->input_colorimage_data(msg_colorimage);
    this->input_depthimage_data(msg_depthimage);
    if (flag_set_tip && flag_set_colorimage && flag_set_depthimage)
        this->calc();
}

void Gridmap::input_tip_data(const std::shared_ptr<const geometry_msgs::msg::TransformStamped> &msg_tip)
{
    tip.position.x = (float)msg_tip->transform.translation.x;
    tip.position.y = (float)msg_tip->transform.translation.y;
    tip.position.z = (float)msg_tip->transform.translation.z;
    tip.quaternion.x = (float)msg_tip->transform.rotation.x;
    tip.quaternion.y = (float)msg_tip->transform.rotation.y;
    tip.quaternion.z = (float)msg_tip->transform.rotation.z;
    tip.quaternion.w = (float)msg_tip->transform.rotation.w;
    flag_set_tip = true;
}

void Gridmap::input_colorimage_data(const std::shared_ptr<const sensor_msgs::msg::Image> &msg_image)
{
...
void Gridmap::input_depthimage_data(const std::shared_ptr<const sensor_msgs::msg::Image> &msg_image)
{
    cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(msg_image);
