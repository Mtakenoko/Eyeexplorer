...
void callback(const std::shared_ptr<const sensor_msgs::msg::Image> & msg_image_L, const std::shared_ptr<const sensor_msgs::msg::Image> & msg_image_R
    ...
    if(dmatch.size() > 5){  //５点アルゴリズムが行えるのに十分対応点があれば
        ...
        double fovx = 1346.490895, fovy = 1363.852516, u0 = 302.869491, v0 = 291.23;
        ...
        for (size_t i = 0; i < dmatch.size(); i++) {	//特徴点の数だけ処理
            ...
            //ip.at<double>(0) = keypoints_L[dmatch[i].queryIdx].pt.x;	//1枚目の画像の特徴点のx座標を取得
            //ip.at<double>(1) = keypoints_L[dmatch[i].queryIdx].pt.y;	//1枚目の画像の特徴点のy座標を取得
            ...
            //ip.at<double>(0) = keypoints_R[dmatch[i].trainIdx].pt.x;	//2枚目の画像の特徴点のx座標を取得
            //ip.at<double>(1) = keypoints_R[dmatch[i].trainIdx].pt.y;	//2枚目の画像の特徴点のy座標を取得
            //ip.at<double>(2) = 1.0;
        ...
        const double deg = 0 * CV_PI / 180;
        Rt1 = cv::Mat::eye(3, 4, CV_64FC1); //片方は回転、並進ともに0の外部パラメータ
        Rt2 = (cv::Mat_<double>(3, 4) << cos(deg), -sin(deg), 0, sin(deg), cos(deg), 0., 0., 0., 1., 70., 0., 0.);
...
int main(int argc, char * argv[])
{
    ...
    sync.registerCallback(std::bind(&callback, std::placeholders::_1, std::placeholders::_2, show_camera, feature, match, prjMat, node_logger, pub, pub_pointcloud, node));
